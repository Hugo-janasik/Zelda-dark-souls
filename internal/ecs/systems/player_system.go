...
// updateMovement met à jour le mouvement du joueur (identique)
...
// vectorToDirection convertit un vecteur en direction (identique)
...
// applyScreenBounds limite le joueur aux bords de l'écran (identique)
...
// updateAnimation met à jour les animations (modifié pour les sprites)
...
// updateCamera met à jour la caméra pour suivre le joueur (identique)
...
func (ps *PlayerSystem) renderWithSprites(renderer Renderer) bool {
	...
	if spriteRenderer, ok := renderer.(interface {
		...
		// TODO: Obtenir le bon sprite depuis PlayerSprites selon l'animation actuelle
		// Pour l'instant, utiliser le sprite principal
...
// renderFallback rendu rectangulaire de fallback (identique à l'ancien système)
...
// renderDirectionIndicator dessine un indicateur de direction (identique)
...
// renderHealthBar dessine une barre de vie (identique)
...
// renderStaminaBar dessine une barre de stamina (identique)
...
func minByte(a, b uint8) uint8 {
	...
	return b
}// internal/ecs/systems/player_system.go - Système de joueur sans imports cycliques